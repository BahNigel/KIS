...
const handleFilePicker = async ({ selectedFiles, certificateForm, setSelectedFiles, setCertificateForm }: handleFilePickerProps) => {
  try {
    ...
    setCertificateForm({ ...certificateForm, media: updatedFiles });
  } catch (error) {
...
export const handleRemoveFile = ({ uri, selectedFiles, certificateForm, setSelectedFiles, setCertificateForm }: handleRemoveFileProps) => {
  const updatedFiles = selectedFiles.filter((file) => file.uri !== uri);
  setSelectedFiles(updatedFiles);
  setCertificateForm({ ...certificateForm, media: updatedFiles });
};

export const handleMediaTypeChange = ({ type, setMediaType, setCertificateForm, certificateForm, setMediaLink }: handleMediaTypeChangeProps) => {
  setMediaType(type);
  setCertificateForm({ ...certificateForm, media: '', mediaType: type });
  if (type === 'link') {
    setMediaLink('');
  }
};
